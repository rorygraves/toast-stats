...
| Remaining-to-minimum counts | Analytics layer for the three absolute "remaining to minimum Distinguished" counts — `paymentsRemaining` = `max(0, ceil(paymentBase×1.01)−totalPayments)`, `paidClubsRemaining` = `max(0, ceil(paidClubBase×1.01)−paidClubs)`, `distinguishedClubsRemaining` = `max(0, ceil(paidClubBase×0.45)−distinguishedClubs)`. Targets sourced from the Distinguished `TIER_THRESHOLDS` (so a count of 0 matches `meetsThreshold`); distinguished% denominator is `paidClubBase`, not activeClubs (lesson 60). Columns render in #688. (#686) | 🔧 Data ready (UI #688) |
